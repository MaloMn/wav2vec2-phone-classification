...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(wav, start, duration):
        sig = sb.dataio.dataio.read_audio(({
            ...
            "stop": (int(start) * 10 + int(duration)) * int(hparams["sample_rate"] / 1_000)
        }))
        print(int(start) * int(hparams["sample_rate"] / 1_000), (int(start) + int(duration)) * int(hparams["sample_rate"] / 1_000))
        return sig
